...
# --- 0. 網址設定 (請修改為您實際部署的網址) ---
# 本地測試通常是 http://localhost:8501
# 如果部署到 Streamlit Cloud，請填寫 https://your-app.streamlit.app
# ⚠️ 注意：手機要掃描電腦的 localhost QR Code，手機與電腦需在同一網域並使用電腦 IP，或使用 ngrok 等工具
...
def add_mock_inventory(num_boxes_target=10): 
    ...
    try:
        ...
        conn.commit()
        st.toast(f"已生成足夠配對 {num_boxes_target} 箱的現貨庫存！ (共 {added_count} 筆)", icon="🥬")
    except Exception as e:
        st.error(f"新增庫存失敗: {e}")
    finally:
        conn.close()
...
def add_future_mock_inventory(num_boxes_target=10):
    ...
    try:
        ...
        conn.commit()
        st.toast(f"已生成下週 (7天後) 的預計產能庫存！ (共 {added_count} 筆)", icon="🌱")
    except Exception as e:
        st.error(f"新增期貨失敗: {e}")
    finally:
        conn.close()
...
def get_weather_mock(tribe):
    weathers = [
        {"desc": "☀️ 晴朗", "temp": "24°C", "rain": "0%", "icon": "☀️"},
        {"desc": "☁️ 多雲", "temp": "21°C", "rain": "10%", "icon": "☁️"},
        {"desc": "🌧️ 小雨", "temp": "18°C", "rain": "60%", "icon": "🌧️"},
        {"desc": "⛈️ 雷雨", "temp": "16°C", "rain": "90%", "icon": "⛈️"}
    ]
...
def render_consumer_portal(sub_id):
    ...
    st.subheader("📅 預約下次配送")
    st.caption("請告知您方便收貨的時間，方便農民安排採收。")
    
    current_next = pd.to_datetime(next_date).date() if next_date else date.today() + timedelta(days=7)
    
    with st.form("next_delivery_form"):
        new_date = st.date_input("期望配送日期", value=current_next, min_value=date.today())
        submit_date = st.form_submit_button("確認預約", use_container_width=True)
        
        if submit_date:
            if update_next_delivery(sub_id, new_date):
                st.success(f"✅ 已更新！我們將安排於 {new_date} 出貨。")
                time.sleep(1.5)
                st.rerun()
    ...
    # Section 2: 本次滿意度回饋
    st.markdown("<div class='consumer-card'>", unsafe_allow_html=True)
    st.subheader("📝 本次滿意度回饋")
    st.caption("您的回饋是農民進步的動力！")
    
    with st.form("feedback_form"):
        rating = st.slider("整體滿意度 (1-5分)", 1, 5, 5)
        comment = st.text_area("給農民的話 (選填)", placeholder="蔬菜很新鮮，謝謝！")
        submit_feedback = st.form_submit_button("送出回饋", type="primary", use_container_width=True)
...
with st.sidebar:
    ...
    st.markdown("---")
    menu = st.radio("功能選單", ["👨‍🌾 農民種植端", "📊 戰情總覽 (Dashboard)", "📦 訂單與媒合 (OMS)", "⚙️ 系統設定"], index=0)
    st.markdown("---")
    st.markdown("### 🛠️ 快速工具")
    if st.button("🔄 修復資料庫 (Init DB)", use_container_width=True):
        ...
        time.sleep(1)
        st.rerun()
    if st.button("🗑️ 清空演示數據 (Reset)", type="primary", use_container_width=True):
        st.session_state['show_reset_overlay'] = True
        st.rerun()
    if st.button("🛠️ 快速新增庫存 (10箱現貨)", use_container_width=True):
        add_mock_inventory()
        time.sleep(1)
        st.rerun()
...
if st.session_state.get('show_reset_overlay', False):
    ...
    with col_center:
        st.markdown("""<div style='background:white;padding:40px;border-radius:15px;text-align:center;border-top:10px solid #d32f2f;margin-top:100px;'><h2 style='color:#d32f2f;'>⚠️ 確認清空？</h2><p style='color:#555;'>將刪除所有<b>種植紀錄</b>與<b>訂單資料</b>。<br>(客戶與農民資料將保留，苗種盤數將重置)</p></div>""", unsafe_allow_html=True)
        c1, c2 = st.columns(2)
        with c1:
...
# === 頁面 2: 戰情總覽 (Dashboard) ===
    ...
    with tab_overview:
        ...
        if not urgent_harvest.empty:
            st.warning(f"⚠️ **緊急採收通知**：有 **{len(urgent_harvest)}** 筆作物需在 3 天內採收！")
            ...
            if not display_df.empty:
                ...
                for _, row in display_df.iterrows():
                    ...
                    with st.container(border=True):
                        ...
                        # [關鍵修改] 使用真正的 App 連結
                        mock_url = f"{BASE_URL}?role=consumer&sub_id={row['subscription_id']}"
